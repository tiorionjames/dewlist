...
export default function AdminDashboard() {
  ...
  if (!user || user.role !== 'admin') {
    navigate('/');  // If a non-admin tries to access
    return null;
  }
      ...
      {loading ? (
        ...
        <>
          ...
          <div>
            ...
            <p className="text-lg mb-2">Users: <span className="font-bold">4 (example)</span></p>
            <p className="text-lg mb-2">Total Tasks: <span className="font-bold">{auditLogs.length}</span> (via logs)</p>
            {/* Add more fake or real stats here if you want */}
          </div>
        </>
      )}
    </div>
